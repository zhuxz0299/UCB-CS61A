...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_pas):
        nonlocal balance, fail
        if fail == 3:
            return "Too many incorrect attempts. Attempts: {}".format(error_pas)
        if input_pas == password:
            if amount > balance:
                return 'Insufficient funds'
            balance -= amount
            return balance
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    print("DEBUG:next_a={},next_b={}".format(next_a, next_b))
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    def joint_acc(amount, password):
        if password == new_pass:  # right new password
            return withdraw(amount, old_pass)
        else:  # wrong new password,but may be the old password
            return withdraw(amount, password)

    feedback=withdraw(0,old_pass)
    if type(feedback) == str:  # wrong password
        return feedback
    else:  # right password
        return joint_acc
...
def remainders_generator(m):
    ...
    "*** YOUR CODE HERE ***"
    def gen(re):
        yield from filter(lambda x:x%m==re,naturals())
    for re in range(m):
        yield gen(re)
