...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_change_with_max(total, max):
        if total < 0:
            return 0
        elif max == 1:
            return 1
        else:
            return count_change_with_max(total-max, max)+count_change_with_max(total, max//2)

    def get_max(total, cur_max):
        if 2*cur_max > total:
            return cur_max
        else:
            return get_max(total, 2*cur_max)

    return count_change_with_max(total, get_max(total, 1))
# TODO a queer solution
...
def move_stack(n, start, end):
    ...
    if n == 1:
        ...
        move_stack(n-1, start=start, end=6-start-end)
        print_move(origin=start, destination=end)
        move_stack(n-1, start=6-start-end, end=end)
