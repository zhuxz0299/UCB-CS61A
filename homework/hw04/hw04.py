...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t) == find_value:
        ...
        return tree(label=label(t), branches=[replace_leaf(b, find_value, replace_value) for b in branches(t)])
...
def has_path(t, phrase):
    ...
    "*** YOUR CODE HERE ***"
    if label(t) == phrase[0]:  # the label of node must match the phrase[0]
        if len(phrase) == 1:  # the phrase comes to an end
            return True
        else:  # the phrase still has some characteristics
            for b in branches(t):  # if t is leaf ,this will not be excuted and return False
...
def quadratic(x, a, b, c):
    ...
    f1 = func(lower_bound(x))
    f2 = func(upper_bound(x))
    f3 = func(-b/(2*a))
    if debug:
        print("DEBUG:f1={},f2={},f3={}".format(f1,f2,f3))
