...
def is_swap(player_score, opponent_score):
    ...
    opponent_tens_digit, opponent_ones_digit = opponent_score//10 % 10, opponent_score % 10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    pre_score0, pre_score1 = 0, 0  # record the pre_score to apply the feral hog
    while score0 < goal and score1 < goal:
        if who:  # when player 1 take turn
            pre_score1, score1, score0 = get_score(
                pre_score1, score1, score0, strategy1, dice, feral_hogs)
        else:  # when player 0 take turn
            pre_score0, score0, score1 = get_score(
                pre_score0, score0, score1, strategy0, dice, feral_hogs)
        if debug:
            print("DEBUG:score0={},score1={}".format(score0, score1))
            print("DEBUG:----------------------------")
        who = other(who)
...
# opponent is the opponent's score
def get_score(pre_score, score, opponent, strategy, dice, feral_hogs):
    # get the dice_number by the strategy
    dice_number = strategy(score, opponent)
    # get the score of this turn
    cur_score = take_turn(dice_number, opponent, dice)
    score += cur_score
    if feral_hogs and abs(dice_number-pre_score) == 2:  # feral hogs
        score += 3
    if is_swap(score, opponent):  # swap the score
        tmp = score
        score = opponent
        opponent = tmp
    # return current socre as previous score of next turn
    return cur_score, score, opponent
