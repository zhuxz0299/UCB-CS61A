...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operands_val= [(operand.eval(env) if (type(operand)==Name and operand.var_name in env) else operand.eval(global_env)) for operand in self.operands]
...
class PrimitiveFunction(Value):
    ...
    def apply(self, arguments):
        ...
        try:
            out= self.operator(*[arg.value for arg in arguments])
            return Number(out)
